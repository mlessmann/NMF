    ...
    public abstract class Transformation
    {
        ...
        public bool IsInitialized
        {
            ...
            protected set
            {
                if (value)
                {
                    if (!isRulesCreated || !isRulesRegistered) throw new InvalidOperationException(ErrorStrings.TransformationSetIsInitializedNoRulesRegistered);
                }
                isInitialized = value;
            }
        }
        ...
        public bool IsRulesRegistered
        {
            ...
            protected set
            {
                if (value && !isRulesCreated) throw new InvalidOperationException(ErrorStrings.TransformationSetIsRulesInitializedNoRulesCreated);
        ...
        protected void CreateRulesInternal()
        {
            if (!isRulesCreated)
            {
                ...
                if (rules == null) throw new InvalidOperationException(ErrorStrings.TransformationCreateRulesNull);
        ...
        public virtual IEnumerable<GeneralTransformationRule> GetRulesForTypeSignature(Type[] inputTypes, Type outputType)
        {
            ...
            return Rules.Where(rule => inputTypes.IsAssignableArrayFrom(rule.InputType) && (rule.OutputType == outputType || outputType.GetTypeInfo().IsAssignableFrom(rule.OutputType.GetTypeInfo())));
        }
        ...
        public virtual GeneralTransformationRule GetRuleForTypeSignature(Type[] inputTypes, Type outputType)
        {
            ...
            return Rules.FirstOrDefault(rule => inputTypes.IsAssignableArrayFrom(rule.InputType) && (rule.OutputType == outputType || outputType.GetTypeInfo().IsAssignableFrom(rule.OutputType.GetTypeInfo())));
        }
