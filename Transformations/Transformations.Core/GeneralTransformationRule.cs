...
namespace NMF.Transformations.Core
{
    ...
    public abstract class GeneralTransformationRule
    {
        ...
        private void AddBase(GeneralTransformationRule rule, Predicate<Computation> filter)
        {
            ...
            {
                throw new InvalidOperationException(ErrorStrings.MarkInstantiatingForOneRuleAtMost);
            }
        }
        ...
        public bool IsInstantiating(Computation computation)
        {
            if (computation != null)
            {
                ...
                return HasCompliantInput(computation)
                    && (rule.OutputType == OutputType || rule.OutputType.GetTypeInfo().IsAssignableFrom(OutputType.GetTypeInfo()))
        ...
        public bool HasCompliantInput(Computation computation)
        {
            ...
            for (int i = 0; i < inputTypes.Length; i++)
            {
                ...
                if (instance != null && !inputTypes[i].GetTypeInfo().IsAssignableFrom(instance.GetType().GetTypeInfo())) return false;
            }
            return true;
        }
        ...
        {
            if (rule == null) throw new ArgumentNullException("rule");
            if (rule.InputType.IsAssignableArrayFrom(InputType) && (rule.OutputType == OutputType || rule.OutputType.GetTypeInfo().IsAssignableFrom(OutputType.GetTypeInfo()) || OutputType.GetTypeInfo().IsInterface))
            {
                MarkInstantiatingFor(rule, null);
            }
            else
            {
                throw new InvalidOperationException(ErrorStrings.MarkInstantiatingForMustInherit);
            }
        }
