    ...
    internal abstract class ModelExpressionVisitorBase<TState> : ExpressionVisitorBase
    {
        ...
        public class PropertyAccess : PropertyChainNode
        {
            ...
            public Type Anchor
            {
                get
                {
                    return Property.GetCustomAttribute<AnchorAttribute>(true)?.AnchorType;
                }
            }
            ...
            public bool IsAnchorEffective(Type anchor)
            {
                if (anchor != null)
                {
                    ...
                    while (current != null)
                    {
                        if (anchor.GetTypeInfo().IsAssignableFrom(current.Type.GetTypeInfo()))
                        {
                            return false;
                        }
        ...
        {
            if (method.GetCustomAttribute<ImmutableMethodAttribute>(true) != null) return true;
        ...
        {
            if (type.GetTypeInfo().IsValueType || type == typeof(string) || type == typeof(Uri)) return true;
            var immutableObjectAttribute = type.GetTypeInfo().GetCustomAttribute<ImmutableObjectAttribute>(false);
            ...
            return HasAttribute<CompilerGeneratedAttribute>(type.GetTypeInfo(), false);
        }

        private static bool IsContainmentProperty(PropertyInfo property)
        {
            if (property.DeclaringType.GetTypeInfo().IsValueType) return true;
            return HasAttribute<ContainmentAttribute>(property, true);
        }

        private static bool HasAttribute<TAttribute>(MemberInfo member, bool inherit) where TAttribute : Attribute
        {
        ...
        protected override Expression VisitMember(MemberExpression node)
        {
            ...
            if (property != null && IsModelProperty(property))
            {
                ...
                var isCrossReference = !isContainment && typeof(IModelElement).GetTypeInfo().IsAssignableFrom(property.PropertyType.GetTypeInfo());
        ...
        protected virtual bool IsModelProperty(PropertyInfo property)
        {
            ...
            return typeof(IModelElement).GetTypeInfo().IsAssignableFrom(property.DeclaringType.GetTypeInfo());
        }


        private static bool IsGenericEnumerableExpression(Type type)
        {
            return type.GetTypeInfo().IsInterface && type.GetTypeInfo().IsGenericType && type.GetGenericTypeDefinition() == typeof(IEnumerableExpression<>);
        }
        ...
        private static Type FindEnumerableExpressionType(Type type)
        {
            ...
            return type.GetTypeInfo().ImplementedInterfaces.FirstOrDefault(IsGenericEnumerableExpression);
        }
        ...
        {
            var state = SaveState();
            var remaps = node.Method.GetCustomAttributes<ParameterDataflowAttribute>(true);
